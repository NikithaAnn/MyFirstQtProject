...
//  ngl::Vec3 from(0,1,-4);
  ...
  shader->setUniform("material.diffuse",0.890f, 0.854f, 0.788f,0.0f);
...
void NGLScene::buildVAO()
{
  ...
  n=ngl::calcNormal(verts[20],verts[19],verts[18]);
  verts.push_back(n);
  verts.push_back(n);
  verts.push_back(n);
...
void NGLScene::paintGL()
{
  ...
  m_transform.reset();
  m_transform.setRotation(0,0,m_teapotRotation);
  ...
  M=m_mouseGlobalTX;
  MV=  m_view*M;
  MVP= m_project*MV*m_transform.getMatrix();
  normalMatrix=MV;
